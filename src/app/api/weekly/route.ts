...
import { getWeeklyCached, setWeeklyCached } from "@/lib/cache";
  ...
  return Sentry.startSpan(
    ...
    async (span) => {
      ...
      try {
        ...
        try {
          await setWeeklyCached({
            cityId,
            years,
            data: allWeeklyData,
          });
        } catch (err) {
          console.error("Failed to cache weekly data in SQLite:", err);
